...
c = GetProp(hwnd, hwnd & "#" & Msg & "C")
If (c > 0) Then
   For iC = 1 To c
      If (GetProp(hwnd, hwnd & "#" & Msg & "#" & iC) = ObjPtr(iwp)) Then
...
If Not (bFail) Then
   ...
   f = SetProp(hwnd, hwnd & "#" & Msg & "C", c)
   Debug.Assert f <> 0
   
   ' Associate object with message at the count:
   f = SetProp(hwnd, hwnd & "#" & Msg & "#" & c, ObjPtr(iwp))
   Debug.Assert f <> 0
End If
End Sub
...
If (c > 0) Then
   ...
   If (iP <> 0) Then
      ' Remove this item:
      For iC = iP + 1 To c
         lPtr = GetProp(hwnd, hwnd & "#" & Msg & "#" & iC)
         SetProp hwnd, hwnd & "#" & Msg & "#" & (iC - 1), lPtr
      Next iC
   End If
   ' Decrement the count
   RemoveProp hwnd, hwnd & "#" & Msg & "#" & c
   c = c - 1
   SetProp hwnd, hwnd & "#" & Msg & "C", c
...
If (iPC > 0) Then
   ...
   For iP = 1 To iPC
      ...
      If Not (bNoProcess) Then
         ...
         With iwp
            'this is slightly changed compared to ssubtmr.dll
            'let the control's window proc do its job
            bHandled = False
            WindowProc = .WindowProc(hwnd, Msg, wParam, ByVal lParam, bHandled)
...
CallOldWindowProc = CallWindowProc(m_iProcOld, hwnd, Msg, wParam, lParam)
